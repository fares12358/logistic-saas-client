  ...
  return (
    <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="flex flex-col gap-5">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-gray-700">{voyage.polId?.name} <span className="text-gray-400 text-xs mono">{voyage.polId?.code}</span></p>
          </div>
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-0.5">Port of Discharge</p>
            <p className="font-medium text-gray-700">{voyage.podId?.name} <span className="text-gray-400 text-xs mono">{voyage.podId?.code}</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={labelCls}>ETD <span className="text-gray-300 font-normal">(Estimated Departure)</span></label>
          <input type="datetime-local" {...register('etd')} className={inputCls(false)} />
        </div>
        <div>
          <label className={labelCls}>ETA <span className="text-gray-300 font-normal">(Estimated Arrival)</span></label>
          <input type="datetime-local" {...register('eta')} className={inputCls(false)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={labelCls}>ATD <span className="text-gray-300 font-normal">(Actual Departure)</span></label>
          <input type="datetime-local" {...register('atd')} className={inputCls(false)} />
        </div>
        <div>
          <label className={labelCls}>ATA <span className="text-gray-300 font-normal">(Actual Arrival)</span></label>
          <input type="datetime-local" {...register('ata')} className={inputCls(false)} />
        </div>
      </div>
