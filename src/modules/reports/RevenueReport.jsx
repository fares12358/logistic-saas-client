...
export default function RevenueReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['report-revenue', serviceId, dateFrom, dateTo],
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="table-header">
                  {['Service Code', 'Service Name', 'Voyages', 'Bookings', 'Units', 'Total Revenue'].map(h => <th key={h}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {rows.map(r => (
                  <tr key={r._id} className="table-row">
                    <td><span className="mono text-xs font-bold text-teal-600">{r.serviceCode}</span></td>
                    <td className="text-sm text-gray-700">{r.serviceName}</td>
                    ...
                    <td><span className="mono text-xs font-bold text-teal-700">{fmtMoney(r.totalRevenue)}</span></td>
                  </tr>
                ))}
                {/* Grand total */}
                <tr className="bg-gray-50 font-semibold border-t-2 border-gray-200">
                  <td colSpan={2} className="px-4 py-2 text-xs text-gray-600">GRAND TOTAL</td>
                  <td className="px-4 py-2 text-xs">{fmtNum(grandTotal.voyageCount)}</td>
                  <td className="px-4 py-2 text-xs">{fmtNum(grandTotal.bookingCount)}</td>
                  <td className="px-4 py-2 text-xs">{fmtNum(grandTotal.totalQty)}</td>
                  <td className="px-4 py-2 mono text-xs font-bold text-teal-700">{fmtMoney(grandTotal.totalRevenue)}</td>
                </tr>
              </tbody>
            </table>
          </div>
